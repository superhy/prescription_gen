...
loss_type <- c(rep("val_loss", time=81), rep("train_loss", times=81))
loss <- c(val_loss, train_loss)
...
sp <- ggplot(data=df, mapping=aes(x=epochs, y=loss, color=loss_type, group=loss_type)) +
		# geom_ribbon(aes(ymin=min, ymax=max), fill="#339999", alpha=0.3) +
		geom_line(size=1.) + 
...
sp <- sp + coord_fixed(ratio=1200/1) + scale_y_continuous(limits=c(0.05, 0.11), breaks=c(0.05, train_loss_minBreak, 0.065, val_loss_minBreak, 0.07, 0.08, 0.09, 0.1, 0.11)) + 
